							...
							if(data.success==0){
								...
								this.props.history.push({
									  pathname: '/broker-agreement-payment',
									    state: {payType:item.paytype, Amount:item.amount,Currency:item.currency, templateId:item.templateId,userId:JSON.parse(this.state.userData).assets_id, loc: '/broker-agreement-templates',Validity:item.validity }
									})
							}
						  }
						).catch((error) => {
                        ...
                        <div className="table-responsive">
                            ...
                            {this.state.templateList.map((item,i)=>(
                                    <tr key={item.templateId}>
										...
										{item.paytype=="Free" || item.paidStatus=='Active'?<Link to={{pathname:'/broker-agreement-send',state:{TemplateId:item.templateId,templateDescription:item.templateDescription,agreement_type:'Template',loc:'/agreement-templates'}}} title="Send" className="table-action-btn view-rqu" ><i className="mdi mdi-redo-variant"></i></Link>:<a  className="table-action-btn" >
