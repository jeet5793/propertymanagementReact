 ...
 {/* <!-- Personal-Information --> */}
				<div className="card-box">
                  ...
                  <div className="panel-body"> <img onError={this.addDefaultSrc} id="single-image" src={item!==''?API_URL+item.img_path:img_not_available} alt="image-1" className="img-fluid" />
                   ...
                   <Link to={{"pathname":"/my-property"}}><a className="btn btn-custom waves-light waves-effect w-md">View more...</a></Link> </div>)):<div>No Property Added</div>}
                </div>
                      ...
                      <table className="table table-hover m-0 table-actions-bar">
						...
						{TenantList.map((item,key)=>(
                          <tr key={item.profile_id}>
                            ...
                            <td><i className="mdi mdi-map-marker text-primary"></i> {item.country}</td>
                            <td>{item.connectedDate}</td>
                            <td><Link to={{"pathname":"/owner-tenant-profile",state:{profileid:item.profile_id,session:JSON.parse(this.state.userData).session_id,loc: this.props.location}}} className="view-icon"><i className="mdi mdi-eye"></i></Link></td>
                          </tr>
						   ))}
                        </tbody>
                      </table>:<div style={{textAlign:'center'}}>No Contact Available</div>}
