		...
		.then(
			(data) => {
				...
				if (data.success) {
					...
					this.props.history.push({pathname:'/owner-agreement-preview',state:{dataToSend:dataToSend,PreviewAgreement:this.state.previewAgreement,loc:'/owner-agreement-send',locCommon:this.props.location.state.loc}});
				}
			},
			(error) => {
				console.log('error')
